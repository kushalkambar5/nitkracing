...
class Title {
  ...
  onResize() {
    ...
    const textWidth = textHeight * this.textAspect;
    this.mesh.scale.set(textWidth / this.plane.scale.x, textHeight / this.plane.scale.y, 1);
    this.mesh.position.set(0, -0.5 - (textHeight * 0.5 + 0.05) / this.plane.scale.y, 0);
  }
}
...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Set height based on scale
    const baseHeight = 750;
    this.plane.scale.y = (this.viewport.height * (baseHeight * this.scale)) / this.screen.height;
    
    // Fit plane width to image aspect ratio
    const aspect = this.imageAspect || 1.0;
