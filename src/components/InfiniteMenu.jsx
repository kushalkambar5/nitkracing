...
// Robust preloader helper that crops & resizes images and video frames to 512x512 squares.
function loadMedia(item, cellSize) {
    ...
    if (item.isVideo) {
      ...
      const timeoutId = setTimeout(() => {
        if (!resolved) {
          ...
          ctx.fillText('Video loading...', cellSize / 2, cellSize / 2);
          resolve(canvas);
        }
      }, 6000);
      ...
      video.onseeked = () => {
        ...
        ctx.drawImage(video, sx, sy, size, size, 0, 0, cellSize, cellSize);
        resolve(canvas);
      };
      ...
      video.onerror = () => {
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('Video Frame Error', cellSize / 2, cellSize / 2);
        resolve(canvas);
      };
    } else {
      const img = new Image();
      ...
      img.onload = () => {
        ...
        ctx.drawImage(img, sx, sy, size, size, 0, 0, cellSize, cellSize);
        resolve(canvas);
      };
...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0, onLoaded = null) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #initTexture(onLoaded) {
    ...
    Promise.all(
      this.items.map(item => loadMedia(item, cellSize))
    ).then(canvases => {
      canvases.forEach((cv, i) => {
        const x = (i % this.atlasSize) * cellSize;
        const y = Math.floor(i / this.atlasSize) * cellSize;
        ctx.drawImage(cv, x, y, cellSize, cellSize);
      });
...
export default function InfiniteMenu({ items = [], scale = 1.0, onItemClick }) {
  ...
  return (
    <div className="infinite-menu-wrapper">
      {isLoading && (
        <div className="infinite-menu-loader">
          <div className="infinite-menu-spinner"></div>
          <p className="infinite-menu-loading-text">Assembling 3D Sphere Gallery...</p>
        </div>
      )}
      ...
      {activeItem && !isLoading && (
        <button 
          ...
          aria-label={`View details for ${activeItem.title}`}
        >
