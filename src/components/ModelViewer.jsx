...
const ModelInner = ({
  ...
  useLayoutEffect(() => {
    ...
    // Center of model is now at g's local origin, making the pivot (0, 0, 0)
    ...
    if (fadeIn) {
      ...
      const id = setInterval(() => {
        ...
        if (v === 1) {
          clearInterval(id);
          onLoaded?.(`Radius: ${sphere.radius.toFixed(4)}, Scale: ${s.toFixed(4)}, Center: [${sphere.center.x.toFixed(2)}, ${sphere.center.y.toFixed(2)}, ${sphere.center.z.toFixed(2)}]`);
        }
      }, 16);
  ...
  useFrame((_, dt) => {
    ...
    // Clamp rotation.x to prevent looking under the model (bottom view)
    // 0.0 is the horizontal plane, and Math.PI / 2.3 limits the top-down view
